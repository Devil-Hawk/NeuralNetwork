...
public class ConvolutionalNode {
                ...
                while (i < batchSize) {
                    ...
                    while (z < sizeZ) {
                        ...
                        while (y < sizeY) {
                            ...
                            while (x < sizeX) {
                                double val = inputs[i][z][y][x] + bias[z][y][x];
                                double cappedVal = Math.min(Math.max(0, val), 5);
                                outputValues[i][z][y][x] = cappedVal;
                                if (val > 0 && val < 5) {
                                    derivative[i][z][y][x] = 1;
                                }
                                x++;
                            ...
                            while (y < sizeY) {
                                ...
                                while (x < sizeX) {
                                    ...
                                    if (testVal < (1.0 - dropoutRate)) {
                                        ...
                                        dropoutDelta[i][z][y][x] = 1 / (1.0 - dropoutRate);
                                    } else {
                                        outputValues[i][z][y][x] = 0;
                                    }
                                    x++;
            ...
            while (r < batchSize) {
                ...
                while (z < sizeZ) {
                    ...
                    while (y < sizeY) {
                        int x = 0;
                        while (x < sizeX) {
                            delta[r][z][y][x] *= derivative[r][z][y][x];
                            biasDelta[r][z][y][x] += delta[r][z][y][x];
                            x++;
                        }
                        y++;
                    }
    ...
    public void initializeWeightsAndBiasKaiming(double initialBias, int fanIn) {
        ...
        int edgeIndex = 0;
        while (edgeIndex < inputEdges.size()) {
            Edge e = inputEdges.get(edgeIndex);
