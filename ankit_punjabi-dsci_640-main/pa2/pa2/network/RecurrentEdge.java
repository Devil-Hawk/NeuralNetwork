...
public class RecurrentEdge {
    ...
    public void propagateBackward(int timeStep, double delta) {
        ...
        int lastTS = timeStep - timeSkip;
        if (lastTS<0)
            return;
        RecurrentNode incomingNode = this.inputNode;
        this.weightDelta += delta* incomingNode.postActivationValue[lastTS];
        incomingNode.delta[lastTS] += delta * this.weight;
    }
